...
@section('breadcrumb','Pedidos / Cotizaciones / Nueva')
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input x-model="coti.serie" type="text" readonly class="w-full rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-sm text-gray-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Número</label>
                            <input x-model="coti.numero" type="text" readonly class="w-full rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-sm text-gray-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Fecha Emisión *</label>
                        <input x-model="coti.fecha" type="date" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Forma de Pago *</label>
                        <select x-model="coti.id_tipo_pago" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                ...
                <div class="space-y-3">
                    ...
                    <template x-if="clienteSeleccionado">
                        <div class="rounded-xl bg-blue-50 border border-blue-200 p-3">
                            ...
                            <button @click="clienteSeleccionado=null;coti.id_cliente=null" class="mt-2 text-[10px] text-red-500 hover:underline">
        ...
        <div class="xl:col-span-2 space-y-4">
            ...
            <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
                <div class="border-b border-gray-100 px-5 py-4 flex items-center justify-between">
                    ...
                    <button x-show="productosCoti.length>0" @click="productosCoti=[];recalcular()" class="text-xs text-red-500 hover:underline">Limpiar</button>
                </div>
...
function cotiApp() {
    return {
        ...
        async init() {
            const d = await apiGet(this.BASE+'/api/cotizaciones/tipo');
            this.documentos = Array.isArray(d) ? d : [];
        },

        seleccionarTido() {
            const doc = this.documentos.find(d=>d.id_tido==this.coti.id_tido);
            if(doc){ this.coti.serie=doc.serie; this.coti.numero=String(parseInt(doc.numero)+1).padStart(8,'0'); }
        },
        ...
        async guardarCotizacion() {
            ...
            try {
                ...
                const data=await apiPost(this.BASE+'/api/cotizaciones/add',payload);
                if(data.res){
