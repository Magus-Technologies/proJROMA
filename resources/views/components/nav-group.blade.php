...
<div x-data="{ open: @js($active) }">
    <button type="button" @click="open=!open"
            ...
            :class="open ? 'text-white' : 'text-blue-100/80'">
        <i class="{{ $icon }} text-sm" :class="open ? 'text-blue-300' : 'text-blue-400/60'"></i>
        <span class="flex-1 truncate text-left text-xs">{{ $label }}</span>
        <i class="ti ti-chevron-down text-xs text-blue-400/60"
           :class="open && 'rotate-180'" style="transition:transform .2s"></i>
    </button>

    <div x-show="open" x-cloak x-collapse.duration.200ms
